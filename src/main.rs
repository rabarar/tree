...
fn model(app: &App) -> Model {
    ...
    println!("fade: {:?}", cli.fade);

    let _window = app
        .new_window()
        .size(cli.wxsize, cli.wysize)
        .view(view)
        .event(window_event)
        .build()
        .unwrap();
    ...
    //let size = 1.0;
    //let frac = 0.0;
    let candidate = Thing::new(cli.xpos, cli.ypos, cli.size, cli.fraction, None);
...
fn update(app: &App, model: &mut Model, _update: Update) {
    ...
    //recreate the tree
    let mut tree = quadtree::QuadTree::new(model.wxsize as f32, model.wysize as f32);
    ...
    for i in 0..max_count {
        ...
        if model.things[i].energy > 10.0 {
            ...
            //or branch
            let prob = model.prob; //0.02 ; // TODO 0.2;
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    let mut frac_end = (((app.elapsed_frames() + 120) as i32 - model.frame as i32) as f32) / 100.0;
    if model.fade == true {
